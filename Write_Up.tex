...
\begin{abstract}
\small{
This paper replicates the article: ``Real Exchange Rate Behaviour:
Evidence from Black Markets'' by \protect\hyperlink{ref-Kul}{Luintel}
(\protect\hyperlink{ref-Kul}{2000}) and adds some robustness checks.
}
\end{abstract}
...
1974:11 there is a mistake in the MNR data 1975:09 there is a mistake in
the PHL data

\hypertarget{introduction-what-i-did-and-why-i-did-it}{%
\section{\texorpdfstring{Introduction \label{Introduction} WHAT I DID
AND WHY I DID
IT}{Introduction  WHAT I DID AND WHY I DID IT}}\label{introduction-what-i-did-and-why-i-did-it}}

In economics, we term some countries as `developed' and others as
developing - but how do we compare living standards and economic
productivity between countries? This is one of the questions that
macroeconomics attempts to answers and a number of tools have been
developed
...
power parity (PPP). PPP is an economic theory that compares different
countries' currencies through a ``basket of goods'' approach A question
that
...
After the introduction (which summarizes what you did and why)
...
\hypertarget{context}{%
\section{\texorpdfstring{Context \label{Con}}{Context }}\label{context}}

provide a brief section that outlines the context and questions of the
replicated study:  The first part should outline what the authors do,
how they motivate the question as of economic interest and/or
importance, how they motivate their methods and how they argue that
their contribution is novel  The second part can be a critical
evaluation of their approach and choices which leads to your choices of
...
Descriptive Statistics table 10 countries: Indian Rupee (IRupee),
Malaysian Ringgit, Myanmar Kyat, Pakistani Philippine Peso, Sri Lankan
Rupee (SRupee), Taiwanese dollar (TDollar) Data span 31 years

Tests Mean reversion is tested by implementing unit-root tests in a
heterogeneous proposed by Im et al.~(1997). The panel unit-root test is
supplemented by a variance ratio test (Cochrane, 1988). In possible
difference in the mean reverting behaviour of real exchange rates
between 1973 periods, we test separately for the pre-float and float
sub-periods in addition period. Structural breaks in real exchange rates
are formally tested through which identifies break dates endogenously
and, where identified, unit-root tests allowing for the brea countries
are stationary) can be generalized. We investigate the mean reverting
behaviour of real exchange rates using a developing countries. The
monthly black market real exchange rates of eight i.e.~used.The data
span is Notice how I used the curly brackets and dash to remove the
numbering of the data section.
