...
datcolat <- Data_Collating(Datroot = "data/files/")
# rm(list = ls(all.names = TRUE)) #will clear all objects includes hidden objects.
# gc()
# cleaned <- datcolat %>% select(-c(5,8,9,12,13,16,17,20,21,24,25,28)) %>% rename( ENTRY = ENTRY...1)
cleaned <- datcolat %>% select(-c(5,9,13,17,21,25)) %>% rename( ENTRY = ENTRY...1)

taw <- read_table("C:/Users/Cassandra/OneDrive/Documents/2021 Academics/Metrics/Time Series Project/R stuffs/Write_Up/data/taw.dat")

final <- full_join(cleaned, taw, by = "ENTRY" , all = TRUE)
view(final)

view(cleaned)

#Splitting the data in the rex file and then joining the dataframe 
rex <-  read_table("C:/Users/Cassandra/OneDrive/Documents/2021 Academics/Metrics/Time Series Project/R stuffs/Write_Up/data/rex.dat")
realexchange1 <- rex %>% slice_head(n = 378)
colnames(realexchange1)
realexchange2 <- rex %>% slice_tail(n = 378) %>% rename(PAKREX = INDREX, PHLREX = LNKREX, TAWREX = MALREX, THIREX = MNRREX)	
view(realexchange2)
realexchange <- bind_cols(realexchange1, realexchange2) %>% select(-c(6)) %>% rename( ENTRY = ENTRY...1)
view(realexchange)
...
runs.test(dfplot$rex)

    #calculating real exchange rate
   #real <- log(EX)-log(CPI)+log(USCPI)
```
...
1974:11 there is a mistake in the MNR data 
1975:09 there is a mistake in the PHL data
...
10 countries: Indian Rupee (IRupee), Malaysian Ringgit, Myanmar Kyat, Pakistani Philippine Peso, Sri Lankan Rupee (SRupee), Taiwanese dollar (TDollar)
Data span 31 years

Tests
Mean reversion is tested by implementing unit-root tests in a heterogeneous proposed by Im et al. (1997). 
The panel unit-root test is supplemented by a variance ratio test (Cochrane, 1988). In possible difference in the mean reverting behaviour of real exchange rates between 1973 periods, we test separately for the pre-float and float sub-periods in addition period. Structural breaks in real exchange rates are formally tested through which identifies break dates endogenously and, where identified, unit-root tests allowing for the brea
countries are stationary) can be generalized. We investigate the mean reverting behaviour of real exchange rates using a developing countries. The monthly black market real exchange rates of eight i.e.  used.The data span is
